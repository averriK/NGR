...
[[ -n "$COMMAND" ]] || fail "This product needs a CLI declaration in install/requirements.R"
...
replace=false
if [[ -n "$installed_version" ]]; then replace=true; fi
if [[ -f "$receipt" ]]; then
  replace=true
...
if [[ "$(id -u)" == 0 ]]; then
  ...
  (
    ...
    publish_cli "$stage_dir" "$PREFIX" "$COMMAND" "$RUNTIME" install "$rscript_path"
  )
else
...
printf 'The R package remains separate from CLI removal. Selected library: %s\n' "$LIBRARY"
