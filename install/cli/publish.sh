...
publish_cli() (
  ...
  for ((i=0; i<${#paths[@]}; i++)); do
    ...
    if [[ "${slots[$i]}" == - ]]; then
      ...
      mv -f -- "$recovery/$i.next" "$target"
      # Staging is prepared by the invoking user; under elevation the classic
      # model gives every published file to root (wheel is group 0 on macOS).
      [[ "$privileged" == false ]] || chown 0:0 "$target"
    fi
  done
