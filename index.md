...
Pkgdown is configured by `_pkgdown.yml` and builds the site into
`docs/`, following the `gmsp`/`newmark` package-site layout.

Pkgdown article sources:

- `vignettes/quarto-yaml.Rmd` — package-facing vignette for the Quarto
  YAML helper API.
- `vignettes/secondary-y-axis.Rmd` — primary/secondary Y axis
  configuration.
- `vignettes/themes-gridlines.Rmd` — `hc_theme_*_gridlines()` reference.
- `vignettes/adding-web-fonts-typewriter.Rmd` — maintainer guide for
  typewriter web fonts.
