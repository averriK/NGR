...
## Effects
- origin/main contiene R1–R8, L2, L3, la adopción del kit y los handoffs (HANDOFF-20260918.md, HANDOFF-SUPERVISOR-20260918.md, HANDOFF-PENDIENTES-20260918.md en esta carpeta). reports/sha modificado fuera de git: manifest.json, scripts/setup/cover.R, scripts/setup/transmittal.R.
- `ngr` instalado en /usr/local el 2026-09-18 13:59 -03 desde 36e7182 con árbol SUCIO: BUILD_INFO git_describe termina en -dirty, por lo que todo render con esta instalación sale «· DRAFT» (lib/R/resources.R:160-169, lib/R/quartoRenderStamp.R:44-56). Archivos instalados a nombre de averrik, directorios de root.
- Árbol de trabajo: install/ sincronizado con el canónico pero sin commitear (13 modificados, 4 nuevos), trabajo del agente del instalador. Ajenos, no tocar: dev/plan/cli-fusion/STATE.md (modificado, con seguimiento: mantiene -dirty todo el repo), dev/docs/, dev/SoT/NOTICE-INSTALLER-FAMILY-STANDARD-20260918.md.
...
## Blocker
La instalación vigente sella DRAFT: falta que el agente del instalador commitee install/ y resuelva el alcance de `dirty` (hoy todo el repo, install/cli/manage.R:296-300; un archivo ajeno modificado lo mantiene sucio) y que el propietario reinstale con sudo desde árbol limpio. Detalle en NOTICE-C2-VERIFICATION-20260918.md.

## Next
NEXT-ID {"action":"Tras el commit de install/ por el agente del instalador y la reinstalación sudo del propietario: verificar que /usr/local/bin/ngr --version muestre build sin -dirty, que /usr/local/libexec/ngr/scaffold/BUILD_INFO no termine en -dirty, que los archivos instalados sean de root si el agente del instalador lo corrigió, y que un render de prueba en un proyecto scratch selle Rev.<7hex> sin DRAFT; registrar el resultado.","id":"cli-repair-c2-reverify-clean-install-20260918"}
